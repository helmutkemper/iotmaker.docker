...
func (el *DockerSystem) ContainerCreateAndChangeExposedPort(
	...
	if containerNetwork != nil {
		...
		for name := range containerNetwork.EndpointsConfig {
			if name == "bridge" || name == "host" || name == "none" {
				continue
			}

			portExposedList, err = el.ImageMountNatPortListChangeExposedWithIpAddress(imageId, "", currentPort, changeToPort)
			if err != nil {
				return "", err
			}
		...
		portExposedList, err = el.ImageMountNatPortListChangeExposed(imageId, currentPort, changeToPort)
		if err != nil {
			return "", err
		}
	}
